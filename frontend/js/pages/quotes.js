  ...
  root.innerHTML = `
    <div class="submit-container">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:16px">
        ...
        <button id="refreshBtn" class="text-xs" style="margin-left:auto;padding:8px 16px;border:1px solid var(--gold);border-radius:4px;background:transparent;color:var(--gold);cursor:pointer">换一批</button>
      </div>
  ...
  async function loadQuotes(mode) {
    ...
    try {
      ...
      document.getElementById('quoteGrid').innerHTML = quotes.map(function(q) {
        ...
        var modeBg = q.m === 'original' ? 'var(--purple-soft)' : 'var(--crimson-soft)';
        return '<div class="glass-card" style="padding:20px;display:flex;flex-direction:column;justify-content:space-between">'
          ...
          + '<div style="display:flex;align-items:center;justify-content:space-between"><cite class="text-xs" style="color:var(--muted);font-style:normal">—— ' + esc(q.s || '佚名') + '</cite><span class="text-xs" style="padding:2px 8px;border-radius:99px;font-size:10px;color:' + modeColor + ';background:' + modeBg + '">' + modeLabel + '</span></div>'
          + '</div>';
      }).join('');
